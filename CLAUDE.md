...
```
.
├── index.html              ← the entire artifact (HTML + CSS + SVG + JS, no deps)
...
2. Outputs land in `data/` — inspect them.
3. Hand-update the SVG polyline `points="..."` attributes and the `MONTH_DATA` / `YT_DATA` JS objects in `index.html`. There is no auto-regenerate step.
