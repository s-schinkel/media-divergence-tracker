...
that runs this and commits the result.

## Regenerating the mechanical regions (`build_erika.py`)

The data-bearing regions are wrapped in `<!--GEN:name-->`…`<!--GEN:/name-->` markers and are
regenerated by `scripts/build_erika.py` — you no longer hand-edit them. The analytical PROSE
(summaries, v1-comparison bullets, caveats, topic-cluster table, Tab 2 intro) is NOT marked
and stays hand-authored.

```bash
python3 erika-kirk/scripts/build_erika.py --extract   # rebuild data/display/erika_display.json from index.html
python3 erika-kirk/scripts/build_erika.py             # regenerate the marked regions from that JSON
```

Sourcing (important):
- **Tab 1** (composite chart line+dots+labels, the 12-column stats table, MONTH_DATA numbers)
  is recomputed from the data snapshots via `build_tab1.py` — fully data-driven. Edit the
  GDELT/HF inputs or `build_tab1.py`'s curator dicts, then run `--extract` + build.
- **Tab 2** (idx/raw tables, YT_DATA, CHANNEL_COLOR) is sourced from CAPTURED displayed values
  in `erika_display.json`, because `youtube_pull.json` no longer reproduces the published
  indices (YouTube view counts grew after the page was built; Paramount/Valhalla lack
  per-video data). Edit those captured values in the JSON to change Tab 2.
- **Tab 2 chart SVG geometry + the uncertainty bands / v1 line are NOT regenerated** (verbatim,
  hand-tuned). `erika_display.json` also stores hand-applied bits that aren't rule-derivable
  (yellow peak-rows, which cells are `<strong>`).

Verified: regeneration reproduces the live page pixel-for-pixel (Tab 1 = 0 px diff; Tab 2 =
antialiasing-noise only). Always re-verify in a browser after editing.
